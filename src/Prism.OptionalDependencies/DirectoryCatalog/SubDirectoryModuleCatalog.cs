...
{
    //Sadly I needed to copy the whole DirectoryModuleCatalog source as I needed it to support sub directories.
    //The only difference to DirectoryModuleCatalog is that it supports sub directories and because of this will only load the newest version of each found dll.
    /// <summary>
    ...
    /// </remarks>
    public class SubDirectoryModuleCatalog : ModuleCatalog
    {
        /// <summary>
        ...
        protected override void InnerLoad()
        {
            if (string.IsNullOrEmpty(this.ModulePath))
                throw new InvalidOperationException("Module Path was empty."); //This exception text was exchanged because I do not have the Prism.Resources here

            if (!Directory.Exists(this.ModulePath))
                throw new InvalidOperationException(
                    string.Format(CultureInfo.CurrentCulture, "Directory {0} did not exist.", this.ModulePath)); //This exception text was exchanged because I do not have the Prism.Resources here
        ...
        private class InnerModuleInfoLoader : MarshalByRefObject
        {
            ...
            private static IEnumerable<ModuleInfo> GetNotAlreadyLoadedModuleInfos(DirectoryInfo directory, Type IModuleType)
            {
                ...
                //We also search sub directories which makes problems down below.
                ...
                //This is needed in our custom version as we support sub directories. Therefore multiple dlls of the same library are possible. We just take new newest ones.
                var newestFileInfos = fileInfos.GroupBy(x => x.Name).Select(c => c.OrderByDescending(d => FileVersionInfo.GetVersionInfo(d.FullName).FileVersion).First());

                foreach (FileInfo fileInfo in newestFileInfos)
                {
                    try
                    {
